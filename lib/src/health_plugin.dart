...
class Health {
  ...
  Future<List<ClinicalRecord>> getClinicalRecords({
    ...
    try {
      ...
      if (result is List) {
        return result.map((e) => ClinicalRecord.fromJson(Map<String, dynamic>.from(e as Map))).toList();
      }
      return [];
    } catch (e) {
  ...
  Future<bool> requestClinicalAuthorization(List<ClinicalRecordType> types) async {
    if (!Platform.isIOS) {
      return false;
    }
    ...
    try {
      final result = await _channel.invokeMethod<bool>('requestClinicalAuthorization', args);
      return result ?? false;
    } catch (e) {
      debugPrint('$runtimeType - Exception in requestClinicalAuthorization(): $e');
      return false;
    }
  }

  /// Returns whether Apple Health Records are available on the current device.
  ///
  /// Clinical records require iOS 12+ and are only available when the device
  /// locale supports Health Records, or the user already has downloaded records
  /// in the Health app.
  Future<bool> supportsHealthRecords() async {
    if (!Platform.isIOS) {
      return false;
    }

    try {
      final result = await _channel.invokeMethod<bool>('supportsHealthRecords');
      return result ?? false;
    } catch (e) {
      debugPrint('$runtimeType - Exception in supportsHealthRecords(): $e');
      return false;
    }
  }
  ...
  ///
  /// Returns a best-effort signal that the system no longer needs to show
  /// the authorization sheet for these read-only types.
  ///
  /// Due to HealthKit privacy rules, iOS does not expose a reliable
  /// "read granted" status for clinical-record queries.
  Future<bool> hasClinicalPermissions(List<ClinicalRecordType> types) async {
    ...
    try {
      final result = await _channel.invokeMethod<bool>('hasClinicalPermissions', args);
      return result ?? false;
    } catch (e) {
